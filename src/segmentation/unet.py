...
class Unet(object):
	...
	def generate_output(self,save=True,mode='side_by_side',output_folder='data/outputs/'):
		...
		if save:
			np.save('/content/tmp/pred.npy',predictions)
			predictions = np.load('/content/tmp/pred.npy')

		predictions = predictions.reshape((-1,self.img_rows,self.img_cols))

		out_paths = [''.join([output_folder,name]) for name in names]
		in_paths = [''.join(['/content/data/test/',x]) for x in test_generator.filenames]
...
if __name__ == '__main__':
	...
	u1.generate_output(save=True,mode='mask_only',output_folder='/content/data/outputs/masks_only/')
